...
- **！！！很多同学拿到的Cookie其实是错误的，所以第7步不能省略了很关键**
  ![寻找log.gi](../img/jd4.jpg)
7. 第6步复制出来的cookie比较长，我们只需要`pt_pin=xxxx;`和 `pt_key=xxxx;`部分的内容即可(注:英文引号`;`是必要的)。可以用下面的脚本，在Chrome浏览器按F12，console里面输入下面脚本按enter回车键，如果剪贴板上没有说明你抓的Cookie是错的
    ``` javascript
    // 快捷指令可忽略此步
